...
function getBalance(){
    $.ajax({
        type: 'GET',
        url: 'balance/'+ $('#depositAccount').val(),
...
function getCumulative(){
    $('#amount').on('input',function (){
        if($('#amount').val()===''){
            ...
            var p = parseFloat($('#amount').val()) + parseFloat($('#balanceCf').val())
            $('#totalCf').val(p)
            $('#total').text(p)
    ...
    $('#deposit').click(function (){
        ...
        if($('#depositAccount').val()==='') {
            swal({
                title: 'Alert!',
                type: 'info',
                text: 'Provide an Account to deposit to!',
            ...
            if ($('#transactionId').val() === '') {
                ...
                var form = $("#deposit-form")[0];
                var data = new FormData(form);

                $.ajax({
                    type: 'POST',
                    url: '/protected/accounts/depositMoney',
                    data: data,
                    processData: false,
                    contentType: false
                }).done(function (s) {
...
function getTransactions(){
    $.ajax({
        type: 'GET',
        url: '/protected/accounts/getdepositsgrid',
        processData: false,
        contentType: false,
    }).done(function (s) {
